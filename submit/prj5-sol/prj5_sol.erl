...
% functions.
dept_employees1(Dept, [X|Y]) when Dept == X # employee.dept ->[X|dept_employees1(Dept, Y)];
dept_employees1(Dept, [X|Y]) when Dept =/= X # employee.dept ->[dept_employees1(Dept, Y)];
...
delete_employee(Name, Employees) -> [X||X <- Employees, Name =/= X#employee.name].
...
% [E] appended.
upsert_employee(_, []) -> [];
upsert_employee(E, [E1|E2]) ->
  if
    E#employee.name == E1#employee.name -> [E|upsert_employee(E,E2)];
    true -> [E1|upsert_employee(E,E2)]
  end.
