...
deptEmployees [] _= []
deptEmployees(x:xs) dept = do
  if emplDept x == dept
    then x: deptEmployees xs dept
  else deptEmployees xs dept
...
-- used above reference from internet 
--deptEmployees :: [Employee] -> Department -> [Employee]
--deptEmployees _ _ = error "TODO"
...
employeesSalarySum employees = foldr (+) 0 (map emplSalary employees)
...
quadraticRoots a b c = (t1, t2) where
    t1 = ne + sqrt pd / (2 * a)
    t2 = ne - sqrt pd / (2 * a)
    pd = b * b - 4 * a * c
    ne = - b / (2 * a)
...
charIndexes s c = [ y | (x, y) <- zip s [0..], x == c ]
...
foldTree treeFn leafFn tree = case tree of
 Leaf node -> leafFn node
 Tree node lftsbtr rftsbtr -> treeFn (foldTree treeFn leafFn node) lftsbtr (foldTree treeFn leafFn rftsbtr)
