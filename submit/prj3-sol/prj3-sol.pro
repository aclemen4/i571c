...
dept_employees([],_,[]).
dept_employees([E|Es], Dept, B) :- E \= employee(_,_,Dept,_).
dept_employees([E|Es], Dept, [A|B]) :- E = employee(_,_,Dept,_),dept_employees(Es, Dept, B).
...
employees_salary_sum([], Sum, Sum).
employees_salary_sum([H|T], Accumulator, Sum):- H = employee(_,_,_,Salary),Accumulator1 is Accumulator + Salary,
employees_salary_sum(T,Accumulator1,Sum).

% employee([Salary|List],Sum):- employee(_,_,_,Accumulator1),Sum is Accumulator1 + Salary.
...
list_access([], List, Z).
list_access(Indexes,[],nil):-Indexes \=[]. 
list_access([H|T],[L|Ls],Z):- H>0,X1 is H-1,list_access([X1|T],Ls,Z).
list_access([H|T],[L|_],Z):-H=0,list_access(T,L,Z).
    
%X=list_access(0,[L|Ls],Z),list_access(T,L,Z).
...
count_non_pairs([H|T],Acc,NNonPairs):-
    H \= [_|_],
    Acc1 is Acc + 1,
    count_non_pairs(T,Acc1,NNonPairs).
count_non_pairs([H|T],Acc,NNonPairs):-
    H = [_|_],
    count_non_pairs(H,1,M),
    count_non_pairs(T,Acc,N),
    NNonPairs is M+N.
...
divisible_by(Ints, N, Int) :- member(X,Ints),0 is X mod N.

%divisible([], _, []).
%divisible([H|T], X, [H|T1]) :- H mod X =:= 0, divisible(T, X, T1).
%divisible([H|T], X, T1) :- H mod X =\= 0, divisible(T, X, T1).
%divisible_by([H|T], X, T1) :- H mod X =\= 0, divisible_by(T, X, T1).
...
test(none_divisible_by_3, fail) :-
    divisible_by([4, 16, -25, -22, 5], 3, Int).
